...
function wait_for_network_manager() {
    ...
    if systemctl is-active --quiet NetworkManager; then
        ...
        echo "Continuing despite NetworkManager issues..."
    fi
}
...
function base_setup() {
    ...
    while [ $attempts -gt 0 ]; do
        ...
        echo "waiting for initial setup... attempts left: $attempts"
        ((attempts--))
    done

    if [ $attempts -eq 0 ]; then
        echo "initial setup did not finish in time... continuing anyway"
    fi
