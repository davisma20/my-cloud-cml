#!/bin/bash
# Simplified script to build CML 2.7.0 AMI using Packer
...
echo "Starting Packer build for CML 2.7.0... Log: $LATEST_LOG_FILE"

# Execute Packer build using variable files, always logging output
# Both stdout and stderr are captured
packer build \
  -var-file=secrets.pkrvars.hcl \
  cml-2.7.0.pkr.hcl 2>&1 | tee "$LATEST_LOG_FILE"
...
if [ $BUILD_STATUS -eq 0 ]; then
  ...
  # Extract AMI ID from the *last* build in the manifest (format region:ami-id -> ami-id)
